...
async def audit_compliance_gaps(
    ...
    """Audit the organization for missing compliance training. Returns a shaped gaps summary with
        affected users and missing courses. Use when leadership needs to know who is non-compliant.
        Optionally pass department or region to scope the audit.
...
async def get_user_compliance_status(user_id: str) -> dict[str, Any]:
    """Fetch one user's full compliance status. Returns user details, course-by-course status, and a
        compliance summary. Use when a single learner's posture is needed. Pass user_id from the
        LMS.
...
async def generate_audit_report(
    ...
    """Generate an evidence-backed compliance audit report. Returns a structured report with
        severity breakdown, department analysis, top missing courses, and an evidence hash. Use when
        a leadership or regulatory submission is required. Optionally pass department or region to
        scope it.
...
async def generate_compliance_digest(
    ...
    """Diff a fresh audit against the previous snapshot to show what changed. Returns new gaps,
        resolved gaps, trend, severity breakdown, and an evidence hash. Use when tracking whether
        compliance is improving. Pass db_path for the snapshot history; optionally pass department
        or region.
...
async def sync(db_path: str = "complyos.db") -> dict[str, Any]:
    """Pull LMS data into the local SQLite cache (mutating). Returns connector name and synced
        counts of users, courses, enrollments, and learning records. Use when the local cache is
        stale. Pass db_path for the cache location.
...
async def list_connectors(profile: str | None = None) -> dict[str, Any]:
    """List the configured connector capability matrix (read-only). Returns a matrix of connector
        capabilities without connecting to any LMS. Use when deciding which LMS to target.
        Optionally pass profile (all/workforce/campus).
...
async def check_connector_health() -> dict[str, Any]:
    """Check the LMS connector's health (read-only). Returns status, authentication state, and any
        errors. Use when diagnosing why sync or audit fails. Takes no parameters.
...
async def validate_assignment_rule(
    ...
    """Validate an assignment rule before deployment. Returns a validation result with a valid flag,
        issues list, and preview. Use when drafting a new rule. Pass name, target_criteria, and
        course_ids from the rule draft.
...
async def preview_assignment_rule(
    ...
    """Preview which users an assignment rule would affect. Returns affected users, missing courses,
        and a total enrollment count. Use when sizing a rule's impact before deploy. Pass name,
        target_criteria, and course_ids from the rule draft.
...
async def remediate_compliance_gaps(
    ...
    """Audit then remediate compliance gaps in one mutating operation. Returns a summary of gaps
        found and remediation actions taken. Use when closing gaps with reminders, auto-enroll, or
        manager alerts. Pass the same scoping as a prior audit_compliance_gaps call plus the
        remediation toggles.
...
async def export_audit_report_html(
    ...
    """Export an audit report to a styled HTML file. Returns the written file path and a report
        summary. Use when a shareable artifact is needed. Pass output_path for the target file;
        optionally pass department or region to scope it.
...
async def export_compliance_dashboard(
    ...
    """Generate a self-contained HTML compliance dashboard from the current audit and snapshot
        history (read-only export). Returns the dashboard path and summary stats. Use when
        presenting trends to leadership. Pass output_path and db_path; optionally pass department or
        region.
...
async def send_notification(
    ...
    """Send a custom external email notification (mutating). Returns a dict with sent (boolean) and
        optional error. Use when an authorized operator surface must email out. Pass to_address,
        subject, and body from the notification request.
...
async def check_readiness(db_path: str = "complyos.db") -> dict[str, Any]:
    """Check enterprise/school readiness controls without making compliance claims (read-only).
        Returns readiness posture, control statuses, a global watchlist, and forbidden claim
        language. Use when assessing posture pre-audit. Pass db_path for the local cache.
...
async def preview_import_batch(
    ...
    """Preview and quarantine CSV rows before promotion (read-only). Returns a batch id, validation
        counts, issues, and a can_promote flag. Use when ingesting new CSV data. Pass csv_text from
        the source export; optionally pass source_system, profile, and db_path.
...
async def promote_import_batch(
    ...
    """Promote a validated import batch into active learning records (mutating). Returns promotion
        status, promoted and blocked row counts, and an evidence id. Use when a previewed batch is
        cleared. Pass batch_id from a prior preview_import_batch result.
...
async def decide_import_row(
    ...
    """Record an explicit decision on a quarantined import row (mutating metadata). Returns the
        decision result and resulting row status. Use when clearing flagged rows during promotion.
        Pass batch_id and row_id from a prior preview_import_batch result plus decision_type.
...
async def list_evidence_ledger(
    ...
    """List evidence ledger entries and their hashes (read-only). Returns ledger entries for the
        tenant. Use when proving an audit's evidence chain. Pass tenant_id to scope and limit to cap
        rows.
...
async def propose_field_mapping(
    ...
    """Suggest CSV/header-to-schema mappings as a stored, non-mutating proposal. Returns a proposal
        id, suggested mappings, hashes, and provenance. Use when normalizing an unknown CSV layout.
        Pass headers from the source file's header row.
...
async def approve_ai_proposal(
    ...
    """Approve an AI proposal record (mutating metadata only, no compliance-truth change). Returns
        the approved proposal metadata and provenance. Use when a human confirms a proposal. Pass
        proposal_id from a prior propose_field_mapping result.
...
async def collect_security_evidence(
    ...
    """Collect a readiness-only security/SOC2 evidence packet for auditor review. Returns the
        evidence packet model. Use when assembling an auditor packet. Optionally pass period and
        profile.
    """
    context = _mcp_context(track=profile)
    return (
...
async def collect_governance_packet(
    ...
    """Collect a readiness-only AI, HR-boundary, and school governance packet. Returns the
        governance packet model. Use when assembling a governance audit packet. Optionally pass lane
        and profile.
    """
    context = _mcp_context(track=profile)
    return (
...
async def create_privacy_request(
    ...
    """Open a privacy/data-subject request case. Returns the privacy request metadata. Use when a
        subject invokes access, export, correction, deletion, restriction, or objection. Pass
        subject_id and request_type from the subject's request.
...
async def export_privacy_subject(
    ...
    """Export subject data scoped to a privacy request. Returns the exported subject data. Use when
        fulfilling an approved access/export request. Pass request_id from a prior
        create_privacy_request result.
    """
    context = _mcp_context(track=profile)
    return (
        PrivacyProgramService(LocalRepository(db_path))
...
async def approve_privacy_request(
    ...
    """Record controller approval before exporting or deleting subject data. Returns the approval
        metadata. Use when a human controller signs off. Pass request_id from a prior
        create_privacy_request result.
    """
    context = _mcp_context(track=profile)
    return (
...
async def delete_privacy_subject(
    ...
    """Delete subject data unless blocked by an active legal hold. Returns the deletion result. Use
        when fulfilling an approved deletion request. Pass request_id from a prior
        approve_privacy_request result.
    """
    context = _mcp_context(track=profile)
    return (
        PrivacyProgramService(LocalRepository(db_path))
...
async def create_legal_hold(
    ...
    """Create an active legal hold that blocks deletion workflows. Returns the legal-hold record.
        Use when litigation or regulation requires preservation. Pass subject_id, scope, and reason
        from the preservation request.
    """
    context = _mcp_context(track=profile)
    return (
        PrivacyProgramService(LocalRepository(db_path))
...
async def release_legal_hold(
    ...
    """Release a legal hold so deletion workflows can proceed. Returns the release result. Use when
        a preservation reason ends. Pass hold_id from a prior create_legal_hold result.
    """
    context = _mcp_context(track=profile)
    return (
        PrivacyProgramService(LocalRepository(db_path))
...
async def configure_privacy_retention(
    ...
    """Configure tenant retention settings for privacy program evidence. Returns the retention
        policy model. Use when setting how long each artifact class is kept. Pass the per-class day
        counts from the retention policy.
    """
    context = _mcp_context(track=profile)
    return (
        PrivacyProgramService(LocalRepository(db_path))
...
async def run_privacy_retention(
    ...
    """Run retention cleanup for closed privacy program artifacts. Returns the cleanup result. Use
        when enforcing the retention policy. Pass dry_run=true to preview before deleting.
    """
    context = _mcp_context(track=profile)
    return (
        PrivacyProgramService(LocalRepository(db_path))
...
async def list_attestations(
    ...
    """List recorded AI-use-policy/AI-literacy attestations (read-only). Returns the tenant's
        attestation records. Use when reporting which learners are un-attested. Optionally pass
        user_id or requirement_id to filter.
...
async def record_attestation(
    ...
    """Record that a learner attested to a named AI-use policy version (mutating, human-only).
        Returns the recorded attestation, its learning-record id, and its evidence id. Use when a
        human is recording a real attestation. Pass user_id, requirement_id, and policy_version from
...
async def submit_intake(
    ...
    """Capture a training request as a DRAFT and draft its proposal-only packet. Returns the
        captured request and a draft packet flagging missing info with suggested priority and
        routing. Use when triaging an incoming training ask. Pass title and requester from the ask;
        never confirm scope.
...
async def list_intake(
    ...
    """List the tenant's training intake requests (read-only). Returns the request queue. Use when
        triaging the intake backlog. Optionally pass status (draft/confirmed/withdrawn).
...
async def confirm_intake_scope(
    ...
    """Confirm scope for an intake request, moving DRAFT to CONFIRMED (mutating, human-only).
        Returns the confirmed request with its human approver stamped. Use when a human agrees to
        take on the work. Pass request_id from a prior submit_intake or list_intake result.
...
async def request_roster_snapshot(
    ...
    """Preview a source export into quarantine and draft a proposal-only roster view (non-mutating).
        Returns the captured snapshot and a proposal-only learners-x-items roster view. Use when a
        stakeholder needs a roster picture. Pass label and csv_text from the source export.
...
async def list_rosters(
    ...
    """List the tenant's roster snapshots (read-only). Returns the snapshot queue. Use when triaging
        roster requests. Optionally pass status (draft/approved/withdrawn).
...
async def approve_roster_snapshot(
    ...
    """Approve a roster snapshot and promote its quarantined import into truth (mutating,
        human-only). Returns the approved snapshot with its human approver stamped. Use when a human
        clears a previewed import. Pass snapshot_id from a prior request_roster_snapshot or
        list_rosters result.
