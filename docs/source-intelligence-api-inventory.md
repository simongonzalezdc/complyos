...
| DB review/schedule/outbox path | `source-intel schedule-add`, `source-intel run-scheduled`, `source-intel export-packet`, `notifications drain`, API v1, and `/source-intel/review` | Free/local. | Durable schedules, execution receipts, review decisions, action logs, notification events/deliveries, and export packets without external API credentials. | Add customer-specific cadence and hosted auth when deployment target is chosen. |
...
- Notification outbox delivery is separate from source-intelligence execution; failed hooks do not rewrite source-intel job outcomes.
